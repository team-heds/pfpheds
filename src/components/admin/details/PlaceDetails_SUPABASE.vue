...
export default {
  ...
  async mounted() {
    ...
    // Charger les institutions depuis Supabase
    try {
      await this.institutionsStore.fetchInstitutions();
      const institutionsArray = this.institutionsStore.institutions;
      
      // Convertir le tableau en objet indexé par InstitutionId pour compatibilité
      this.institutions = institutionsArray.reduce((acc, inst) => {
        acc[inst.InstitutionId] = inst;
        return acc;
      }, {});
      
      console.log('✅ Institutions chargées depuis Supabase:', Object.keys(this.institutions).length);
    } catch (error) {
