...
const { authenticate, protectAdminMutations, requireAdmin, requireAnyPermission } = require('./middleware/auth')
...
app.use('/api/institutions', protectAdminMutations, institutionsStoreRoutes);
app.use('/api/communities', protectAdminMutations, communitiesStoreRoutes);
app.use('/api/enseignants', authenticate, requireAdmin, enseignantsStoreRoutes);
app.use('/api/filePhysio', authenticate, filePhysioRoutes);
app.use('/api/hashtags', protectAdminMutations, hashtagStoreRoutes);
app.use('/api/posts', authenticate, postsStoreRoutes);
//app.use('/api/praticiens-formateurs', praticiensFormateursStoreRoutes);
app.use('/api/praticiens', authenticate, requireAdmin, praticiensStoreRoutes);
app.use('/api/resultat-votation', resultatVotationRoutes);
app.use('/api/feedbacka', authenticate, feedbackaRoutes);
app.use('/api/push', authenticate, requireAdmin, pushRoutes);
app.use('/api/ftp', authenticate, requireAdmin, ftpRoutes);
app.use('/api/admin/users', authenticate, requireAdmin, adminUsersRoutes);
app.use('/api/integrations/vimeo', authenticate, requireAnyPermission('editor'), vimeoRoutes);
app.use('/api/integrations/github', authenticate, requireAnyPermission('editor'), githubRoutes);
...
registerCareConversStoreRoutes(app, authenticate);

app.use((error, _req, res, _next) => {
  console.error('[API] Request failed:', error.message)
  if (error.name === 'MulterError' || /Type de fichier non autorisé|Type d’image non autorisé/.test(error.message)) {
