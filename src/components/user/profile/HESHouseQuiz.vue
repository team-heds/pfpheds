    ...
    <div v-if="existingHouse" class="existing-house-message">
      <div class="message-card">
        <div class="house-icon-container">
          <i
            :class="getHouseIcon(existingHouse)"
            class="house-main-icon"
            :style="{ color: getHouseColor(existingHouse) }"
          ></i>
        </div>
        <h2 class="house-name" :style="{ color: getHouseColor(existingHouse) }">
          {{ existingHouse }}
        </h2>
        ...
        <div class="confirmation-message">
          ...
          <p>
            Vous faites partie de la maison <strong>{{ existingHouse }}</strong
            >.
          </p>
          <p>
            Le quiz ne peut être effectué qu'une seule fois pour maintenir l'équilibre entre les
            maisons.
          </p>
        </div>
        <div class="action-buttons">
          ...
          <button
            @click="goToHouseStats"
            class="house-button"
            :style="{ backgroundColor: getHouseColor(existingHouse) }"
          >
    ...
    <div class="quiz-header" v-if="!quizCompleted && !existingHouse">
      ...
      <div class="progress-container">
        ...
        <span class="progress-text"
          >Question {{ currentQuestionIndex + 1 }} sur {{ questions.length }}</span
        >
      </div>
    </div>
    ...
    <div class="result-section" v-if="quizCompleted">
      <div class="result-card">
        <div class="house-reveal">
          <div class="house-icon-container">
            <i
              :class="selectedHouse.icon"
              class="house-main-icon"
              :style="{ color: selectedHouse.color }"
            ></i>
          </div>
          <h2 class="house-name" :style="{ color: selectedHouse.color }">
            {{ selectedHouse.name }}
          </h2>
...
const houses = {
  harmonis: {
    ...
    description:
      "Tu es quelqu'un de stable, paisible et centré. Tu cherches l'harmonie autour de toi, tu aides les autres à se sentir bien sans faire de bruit. Tu sais que l'équilibre soigne.",
    color: '#2E8B57',
    icon: 'pi pi-circle'
  },
  elaris: {
    ...
    description:
      'Tu es clair dans tes idées, tu aimes guider les autres et voir au-delà des apparences. Tu éclaires les chemins, tu transmets des idées avec calme et assurance.',
    color: '#DC143C',
    icon: 'pi pi-sun'
  },
  doloris: {
    ...
    description:
      "Tu ressens profondément ce que les autres vivent. Tu as de la compassion, tu veux comprendre avant d'agir. Tu ne fais pas les choses à moitié.",
    color: '#FFD700',
    icon: 'pi pi-heart'
  },
  solencia: {
    ...
    description:
      'Tu es doux, apaisant, et tu offres ta présence aux autres dans les moments difficiles. Tu sais écouter, rassurer et consoler.',
    color: '#4169E1',
    icon: 'pi pi-moon'
  }
}
...
const calculateHouse = async () => {
  ...
  try {
    ...
    const bestAvailableHouse = await gamificationServiceSupabase.findBestAvailableHouse(
      houseScores,
      50
    )
...
const saveHouseSelection = async () => {
  try {
    ...
    try {
      ...
      const { data, error } = await gamificationServiceSupabase.supabase.rpc('assign_my_house', {
        p_house_id: houseId
      })
      ...
      console.log(
        '✅ Maison sauvegardée dans gamification_data:',
        selectedHouse.value.name,
        'pour utilisateur:',
        userId
      )
